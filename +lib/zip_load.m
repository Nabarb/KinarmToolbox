...
%
%   C3D_DATA = ZIP_LOAD opens all zip files in the currect directory and
...
for x = 1:length(zipfiles)
    ...
    common_data = KinarmToolbox.lib.c3d_load('common.c3d', newArgs{:});
    delete('common.c3d');
    
    % bulk load the c3d files
    zip_data.c3d = KinarmToolbox.lib.c3d_load(c3d_filename, newArgs{:});    
    ...
    if exist('analysis/analysis.c3d', 'file') || exist('analysis\analysis.c3d', 'file')
        cd ('analysis')
        zip_data.analysis = KinarmToolbox.lib.c3d_load('analysis.c3d', newArgs{:});
    end
    ...
    zip_data = KinarmToolbox.lib.correctXTorque(zip_data);
